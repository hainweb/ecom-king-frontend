...
const SliderArrow = ({ direction, onClick }) => (
  <button
    ...
    className={`absolute top-1/2 -translate-y-1/2 ${direction === 'prev' ? 'left-4' : 'right-4'
      } z-10 bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-800 w-10 h-10 rounded-full shadow-lg flex items-center justify-center transition-all duration-200 backdrop-blur-sm group`}
  >
    {direction === 'prev' ? (
      <svg
        className="w-6 h-6 text-gray-800 dark:text-white group-hover:-translate-x-0.5 transition-transform"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
...
const [showFullDescription, setShowFullDescription] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300 mt-12 mb-14">
      ...
      {showNotification && (
        <div
          className="fixed top-16 left-1/2 z-50 max-w-md bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg animate-fade-in flex items-center gap-2 transform -translate-x-1/2"
        >
  ...
  {product.CustomOptions && product.CustomOptions.length > 0 && (
    <div>
      ...
      <select
        ...
        className="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 py-2 px-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:border-blue-400"
      >
